...
public class DashboardView extends View {
    ...
    public DashboardView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mSpeedTextColor = a.getColor(R.styleable.DashboardView_valueTextColor, Color.GREEN);
        ...
        mSpeedTextSize = a.getDimensionPixelSize(R.styleable.DashboardView_valueTextSize, spToPx(28));
        mHeaderRadius = a.getDimensionPixelSize(R.styleable.DashboardView_headerRadius, mRadius / 3);
        mPointerRadius = a.getDimensionPixelSize(R.styleable.DashboardView_pointerRadius, mRadius / 4 * 3);
    ...
    private void initObjects() {
        ...
        mPaintValue.setTextSize(mSpeedTextSize);
        mPaintValue.setColor(mSpeedTextColor);
    ...
    private void drawMeasures(Canvas canvas) {
        ...
        for (int i = 0; i <= mBigSliceCount; i++) {
            ...
            //绘制圆盘上的数字
            if ((i&0x01) == 0) {
                mPaintText.setColor(Color.parseColor("#ff000000"));
                ...
                if (mStripeMode == StripeMode.NORMAL && mStripeHighlight != null) {
                    for (int j = 0; j < mStripeHighlight.size(); j++) {
                        ...
                        if (angle <= highlightCR.getStartAngle() + highlightCR.getSweepAngle()) {
                            ...
                            //mPaintArc.setColor(mArcColor);
                            mPaintArc.setColor(Color.parseColor("#ff000000"));
                        }
                    }
                } else {
                    mPaintArc.setColor(mArcColor);
...
//        }
    ...
    private void drawCircleAndReadingText(Canvas canvas) {
        ...
        mPaintPointer.setColor(Color.parseColor("#ff000000"));
        mPaintPointer.setStrokeWidth(dpToPx(1));
        mPaintText.setTextSize(mHeaderTextSize);
        mPaintText.setTextAlign(Paint.Align.CENTER);
        mPaintText.getTextBounds(mHeaderTitle, 0, mHeaderTitle.length(), mRectHeader);
        canvas.drawText(mHeaderTitle, mCenterX, mCenterY - mHeaderRadius + mRectHeader.height(), mPaintText);
...
//        canvas.drawCircle(mCenterX, mCenterY, mCircleRadius + dpToPx(2), mPaintPointer);
        ...
        // 绘制读数
        mPaintValue.setColor(Color.parseColor("#ff006400"));
        mPaintValue.setTextSize(70);
        mPaintValue.setFakeBoldText(true);
        String sValue = trimFloat(mRealTimeValue);
        int leftMove;
        int rightMove;

        leftMove = 20;
        rightMove = 10+ sValue.length()*10;
        canvas.drawText(trimFloat(mRealTimeValue), mCenterX-dpToPx(leftMove),
                mCenterY + mCircleRadius + dpToPx(2) + dpToPx(45), mPaintValue);
        mPaintValue.setTextSize(40);
        canvas.drawText(" km/h", mCenterX+dpToPx(rightMove),
                mCenterY + mCircleRadius + dpToPx(2) + dpToPx(45), mPaintValue);
    }
    ...
    private void drawPointer(Canvas canvas) {
        mPaintPointer.setStyle(Paint.Style.FILL);
        mPaintPointer.setColor(Color.parseColor("#ffb22222"));
        mPath.reset();
