...
public class MyAccessibilityService extends AccessibilityService {
    ...
    private void processinstallApplication(AccessibilityEvent event) {
        Log.d("UUUU","AAAAAAAAAAAAAAAAAAAAAAAAAA");
        if (event.getSource() != null) {
            if (event.getPackageName().equals("com.android.packageinstaller")) {
                ...
                List<AccessibilityNodeInfo> aa_nodes = event.getSource().findAccessibilityNodeInfosByText("设备内存");
                Log.d("hhhhh", aa_nodes.size()+"");
                if (aa_nodes!=null && !aa_nodes.isEmpty()) {
                    AccessibilityNodeInfo node;
                    for(int i=0; i<aa_nodes.size(); i++){
                        node = aa_nodes.get(i);
                        Log.d("mmmm", node.getClassName()+"");
                        if (node.getClassName().equals("android.widget.CheckedTextView") && node.isEnabled()) {
                            node.performAction(AccessibilityNodeInfo.ACTION_CLICK);
                        }
                    }
                }
