...
public class WarningFlag {
    ...
    public static byte[] getWarningFlag() {
        if (outOfRangeMap.containsKey(CommonInfo.getCurItem()))
            flagArray[WARNING_OUT_RANGE] = outOfRangeMap.get(CommonInfo.getCurItem());
        else
            flagArray[WARNING_OUT_RANGE] = '0';

        IUtil.EnclosureOut = flagArray[WARNING_OUT_RANGE] == (char)1;
