...
/*
 * Created by LiHan 2017-11-28.
 */
public class TcpClientDH extends Thread {
    enum SendRunState {SENDCOACH, SENDSTULOGIN, SENDSTUOUT, SENDPOSITION, SENDTRAIN, SELPHOTO, SENDPHOTO;}
    ...
    public TcpClientDH(String address, int port, Context context) {
        hasBeenCreated = true;
...
//        }
        try {
            ...
            if(client == null || out_put == null){
                Insert_DB.getInstance().insertError(null,null,StringUtils.bytesToHexString(bt));
                return;
            }
    ...
    public void run() {
        ...
        new Thread() {
            ...
            public void run() {
                ...
                while (MessageDefine.TcpSendIsRun) {
//                    Log.e(TAG, "发送数据：");
                                ...
                                switch (sendRunState) {
                                    case SENDCOACH://教练签到
                                        ...
                                        if (info != null) {
                                            ...
                                            //教练签到之后，会清除存储的数据
                                            coahBool = false;
                                            sendRunState = SendRunState.SENDSTULOGIN;
                                            Log.d("TCP", "无教练员信息");
                                        }
                                        break;
                                    case SENDSTULOGIN://学员签到
                                        stuInfo = selectable.getDBStuSignedData(1,0);
                                        if (stuInfo != null) {
                                    ...
                                    case SENDSTUOUT://学院签退
                                        stuInfo = selectable.getDBStuSignedData(2,0);
                                        if (stuInfo != null) {
...
//                                                FileCopyUtil.saveRunningLog("Uploading video for cache saved");
//                                                uploadVideo();//上传视频文件
//                                            }
//                                            break;
                                    default:
...
//                }
    ...
    void uploadVideo() {
        VideoHeadInformation ptInfo;
        ptInfo = myVideoHeadInfo.get(0);
        File myPhoto = new File(ptInfo.getSavePath());
        if (!myPhoto.exists()) {
            Update_DB datebase_update = new Update_DB();
            curVideoHead.photoSerial = ptInfo.getTakeID();
            datebase_update.clearVideoHeadFlag(StringUtils.bytesToHexString(curVideoHead.photoSerial));
            if (myVideoHeadInfo != null && myVideoHeadInfo.size() > 0) {
                myVideoHeadInfo.remove(0);
            }
            if (DEBUG)
                Log.i(TAG, "删除无效照片ID：" + StringUtils.bytesToHexString(curVideoHead.photoSerial));
        } else {
            try {
                curVideoHead.fileSize = getFileSize(myPhoto);
            } catch (Exception e) {
                e.printStackTrace();
                if (DEBUG) Log.i(TAG, "获取照片文件大小失败!");
            }
            curVideoHead.totalPackages = (int) (curVideoHead.fileSize / 512);
            if (curVideoHead.fileSize % 512 != 0) {
                curVideoHead.totalPackages++;
            }
            curVideoHead.upMode = ptInfo.getUpMode();
            curVideoHead.channel = ptInfo.getChannel();
            curVideoHead.size = ptInfo.getSize();
            curVideoHead.eventType = ptInfo.getEventType();//定时拍照
            curVideoHead.stuDrivingNo = ptInfo.getStuDrivingNo().getBytes();
            curVideoHead.stuNumber = ptInfo.getStuNumber().getBytes();
            curVideoHead.stuLoginSerial = ptInfo.getStuLoginNumber();
            curVideoHead.photoSerial = ptInfo.getTakeID();
            curVideoHead.filePath = ptInfo.getSavePath();
            byte[] videoHead = CommonInfo.getVideoHeadInfo(curVideoHead, ptInfo.getGpsInfo());
            byte[] sendBytes = getCmdData(0x0900, GD_CMD_INIT_VIDEO, (byte) 1, videoHead, 0);
            sendTcpBytes(sendBytes);
            transState.setState(0x8900, GD_CMD_INIT_VIDEO, transState.WAIT_ACK);
            Utils.saveRunningLog("Ask to upload video: " + StringUtils.bytesToHexString(curVideoHead.photoSerial));
            if (DEBUG) Log.i(TAG, "请求上传视频文件");
        }
    }
...
//                Thread.sleep(100);
//                if (timeCnt++ > 10) {
//                    FileCopyUtil.saveRunningLog("request ack timeout, send 0307 command");
...
//                            FileCopyUtil.writeFileCreated(AppConfig.RSA_DATA_DIRECTORY, CERTIFICATION_NAME, device_certification, 0, cerfi_index);
            ...
            for (int i = 0; i < paramList; i++) {
                ...
                switch (cssz) {
                    ...
                    case 0x0029://	DWORD	缺省时间汇报间隔，单位为秒(s),>0
                        value = (paramBytes[0] & 0xFF) << 24 | (paramBytes[1] & 0xFF) << 16 | (paramBytes[2] & 0xFF) << 8 | (paramBytes[3] & 0xFF);
                        Context ctx = TcpClientDH.this.context;
...
//    byte[] get8104Data(int sequence, int totalNumber, int packNumber) {
...
//                        break;
//                    //reservaytion
//                    case 9://	DWORD	位置汇报策略，0：定时汇报；1：定距汇报；2：定时和定距汇报
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x20;
...
//                        pdata[index++] = (byte) (value);
//                        break;
//                    case 10://	DWORD	位置汇报方案，0：根据ACC状态；1：根据登录状态和ACC状态，先判断登录状态，若登录再根据ACC状态
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x21;
...
//                        pdata[index++] = (byte) (value);
//                        break;
//                    case 11://	DWORD	驾驶员未登录汇报时间间隔，单位为秒(s),>0
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x22;
...
//                        break;
//                    //reservation            case ://0x0023-0x0026	DWORD	保留
//                    case 12://	DWORD	休眠时汇报时间间隔，单位为秒(s),>0
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x27;
...
//                        pdata[index++] = (byte) (value);
//                        break;
//                    case 13://	DWORD	紧急报警时汇报时间间隔，单位为秒(s),>0
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x28;
...
//                        pdata[index++] = (byte) (value);
//                        break;
//                    case 14://	DWORD	缺省时间汇报间隔，单位为秒(s),>0
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x29;
...
//                        break;
//                    //reservation 0x002A-0x002B	DWORD	保留
//                    case 15://	DWORD	缺省距离汇报间隔，单位为米(m),>0
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x2C;
...
//                        pdata[index++] = (byte) (value);
//                        break;
//                    case 16://	DWORD	驾驶员未登录汇报距离间隔，单位为米(m),>0
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x2D;
...
//                        pdata[index++] = (byte) (value);
//                        break;
//                    case 17://	DWORD	休眠时汇报距离间隔，单位为米(m),>0
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x2E;
...
//                        pdata[index++] = (byte) (value);
//                        break;
//                    case 18://	DWORD	紧急报警时汇报距离间隔，单位为米(m),>0
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x2F;
...
//                        pdata[index++] = (byte) (value);
//                        break;
//                    case 20://	DWORD	终端电话接听策略，0：自动接听；1：ACC ON时自动接听，OFF时手动接听
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x45;
...
//                        pdata[index++] = (byte) (value);
//                        break;
//                    case 21://	DWORD	每次最长通话时间，单位为秒(s),0为不允许通话，0xFFFFFFFF为不限制
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x46;
...
//                        pdata[index++] = (byte) (value);
//                        break;
//                    case 22://	DWORD	当月最长通话时间，单位为秒(s),0为不允许通话，0xFFFFFFFF为不限制
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x47;
...
//
//                    //reservation 0x004A-0x004F		保留
//                    case 23://	DWORD	报警屏蔽字。与位置信息汇报消息中的报警标识相对应，相应位为1则相应报警被屏蔽
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x50;
...
//                        pdata[index++] = (byte) (value);
//                        break;
//                    case 24://	DWORD	报警发送文本SMS开关，与位置信息汇报消息中的报警标识相对应，相应位为1则相应报警时发送文本SMS
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x51;
...
//                        pdata[index++] = (byte) (value);
//                        break;
//                    case 25://	DWORD	报警拍摄开关，与位置信息汇报消息中的报警标识相对应，相应位为1则相应报警时摄像头拍摄
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x52;
...
//                        pdata[index++] = (byte) (value);
//                        break;
//                    case 26://	DWORD	报警拍摄存储标识，与位置信息汇报消息中的报警标识相对应，相应位为1则对相应报警时牌的照片进行存储，否则实时长传
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x53;
//                        value = TelnetParameter.getSaveWarningPhoto();
//                        break;
//                    case 27://	DWORD	关键标识，与位置信息汇报消息中的报警标识相对应，相应位为1则对相应报警为关键报警
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x54;
...
//                        pdata[index++] = (byte) (value);
//                        break;
//                    case 28://	DWORD	最高速度，单位为公里每小时(km/h)
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x55;
...
//                        pdata[index++] = (byte) (value);
//                        break;
//                    case 31://	DWORD	当天累计驾驶时间门限，单位为秒(s)
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x58;
...
//                        break;
//                    //reservation 0x005B-0x006F		保留
//                    case 34://	DWORD	图像/视频质量，1-10,1最好
//                        pdata[index++] = 0x00;
//                        pdata[index++] = 0x70;
...
//                        pdata[index++] = (byte) (value);
//                        break;
//                    case 42://	byte	车牌颜色，按照JT/T415-2006的5.4.12
//                        pdata[index++] = 0x00;
...
////                        pdata[index++] = (byte) sValue.length();
////                        if(i==15) {
////                            System.arraycopy(sValue.getBytes("gbk"), 0, pdata, index, sValue.length());
////                        }else {
////                            System.arraycopy(sValue.getBytes("utf-8"), 0, pdata, index, sValue.length());
////                        }
//                    }
////                    index += sValue.length();
        ...
        try {
            switch (gdCmd) {
                ...
                case 0x8102:
                    ...
                    if (dataContent[0] == 1) {
                        if (DEBUG) Log.i(TAG, "正确应答教练签退");
                    } else {
                        if (DEBUG) Log.i(TAG, "应答教练签退出错");
                    }
                    System.arraycopy(dataContent, 1, ackCoachNumber, 0, ackCoachNumber.length);
                    break;
                case 0x8201:
...
//                        FileInputStream fis = new FileInputStream(file);
//                        int readLen;
//                        FileCopyUtil.saveRunningLog("start send photo, size:" + file.length());
...
//                        }
//                        fis.close();
//                        FileCopyUtil.saveRunningLog("send photo file complished");
                ...
                case GD_UP_REQUEST_AREA:
                    ...
                    if (dataContent[0] == 1) {
                        ...
                        if (false && StringUtils.arrayCompareWithSegment(areasList, tempBytes, 6)) {
                            ...
                            Delete_DB.getInstance().clearFenceData();// 清空电子
                             // 围栏数据
...
//                System.arraycopy(pcData, 0, clearText, 0, index);
    ...
    //是否为网络重连后的补传数据
    private boolean isRepairData = false;
...
//                        System.arraycopy(cData, 0, tempBuf, dataContent.length, cData.length); //追加本照片数据包加密字符串
    ...
    private void play(String txt, long time) {
        if (System.currentTimeMillis() - flagPlayTiem >= time) {
            ...
            SoundManage.ttsPlaySound(TcpClientDH.this.context, txt);
        }
    }
